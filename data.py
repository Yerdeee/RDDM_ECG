...
def get_datasets(
    DATA_PATH = "/tf/yj/ppg2ecg/datasets/", 
    ...
    for dataset in datasets:
        ...
        ecg_train = np.load(DATA_PATH + dataset + f"/lead1_train.npy", allow_pickle=True).reshape(-1, 128*window_size)
        ppg_train = np.load(DATA_PATH + dataset + f"/lead3_train.npy", allow_pickle=True).reshape(-1, 128*window_size)
        
        ecg_test = np.load(DATA_PATH + dataset + f"/lead1_test.npy", allow_pickle=True).reshape(-1, 128*window_size)
        ppg_test = np.load(DATA_PATH + dataset + f"/lead3_test.npy", allow_pickle=True).reshape(-1, 128*window_size)
